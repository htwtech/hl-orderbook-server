...
The handoff from snapshot to live stream is **gapless**. The dump is of the node's *persisted* state, `abci_state.rmp`, which the node rewrites every 10 000 blocks (some twelve minutes); the server reads that state's own height from the first bytes of the file, reads the streaming files from there, and applies them in block order on top of the snapshot once it is installed — the streams are held on their bounded channels meanwhile, nothing is cached in memory. (`visor_abci_state.json` tracks the *head*, up to a persistence period ahead of the persisted state; cutting the replay at that height left every event in between out of the book — hundreds of orders missing and hundreds cancelled-but-present on every start, with nothing to count them.) Right after an install the book is therefore up to twelve minutes behind and catches up at replay speed, a minute or two; the `time` on every frame says where it stands. If the server ever detects that events were provably lost (a corrupt line, an oversized batch, watcher data loss), it marks the book out-of-sync and re-fetches a snapshot: the watchers are restarted at the persisted height, the old book stands still until the new one is installed, and the streams are then applied above it the same way — see `orderbook_desyncs_total` in the metrics.
...
| `--abci-state-path` | auto | Path to `abci_state.rmp`. Auto-detected at `<data-dir>/../hyperliquid_data/abci_state.rmp` (a sibling of `--data-dir`, alongside `visor_abci_state.json`). Passed to `hl-node` in direct mode, and read on the host in both modes for the persisted state's height — the floor the streaming files are read from around every snapshot. Override if your node stores state in a non-standard location |
...
| `--visor-state-path` | auto | Path to `visor_abci_state.json`, which contains the node's head height (not the persisted state's: that comes from `abci_state.rmp` itself). Used to bound what an install covers, and as the fallback floor when the state file cannot be read. Auto-detected at `<data-dir>/../hyperliquid_data/visor_abci_state.json` (the *parent* of `--data-dir`, since `hyperliquid_data/` is a sibling of the `data/` event dir). Override if your visor state is in a non-standard location |
...
2. **Snapshot replay** - the streams are held on their bounded channels while the snapshot is generated and installed, then applied above the snapshot height in block order, making the snapshot-to-stream handoff gapless without caching anything in memory. Events at or below the snapshot height (the node persisted again during the dump) are dropped by height.
...
7. **Block-order merge** - the status and diff streams are read by independent threads and each has a bounded channel of its own; the listener takes from whichever has the lower block at its head, a status before a diff of the same block, so the book sees the node's own order however the two readers race. A reader that runs ahead fills its channel and waits on the send -- back-pressure where it belongs, with a fixed memory bound. In steady state nothing waits. After a stall (a snapshot install, a slow flush) both readers catch up in fixed-size chunks and one stream runs far ahead; before the merge, every New in that stretch waited for a status still on disk, the pending caches overran their caps and were cleared whole -- ten thousand orders that would never rest, after every re-sync. A lone stream (the other's channel is empty) is applied up to 32 blocks past the other's last taken height and waits beyond, unless the other delivers nothing for 10 s while being waited on (dead; the watchdog's case). Lines waiting in the two channels are reported as `orderbook_skew_gate_held`. The pending caps themselves are a memory backstop only (200 000 each): overrunning one evicts the oldest entries down to the cap, still counted as data loss.
8. **Startup in step** - the two book readers begin live tracking together, the one that finished its backfill first waiting for the other, so the live streams do not start a whole backfill apart; the backfill lines themselves go through the same block-order merge as the live ones.
...
| | `orderbook_skew_gate_held` | Lines waiting in the two live book channels -- the readers' lead over the listener; near zero in steady state |
