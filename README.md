...
7. **Block-order merge** - the status and diff streams are read by independent threads; they are now queued and applied in block order, the batch with the lower height first and a status before a diff of the same block, so the book sees the node's own order however the two readers race. In steady state nothing waits. After a stall (a snapshot install, a slow flush) both readers catch up in fixed-size chunks and one stream runs far ahead; before the merge, every New in that stretch waited for a status still on disk, the pending caches overran their caps and were cleared whole -- ten thousand orders that would never rest, after every re-sync. A lone stream (the other has delivered nothing newer) is applied up to 32 blocks past the other's last delivered height and waits beyond, unless the other has been silent for 600 blocks (dead; the watchdog's case). Queued batches are counted in `orderbook_skew_gate_held`. The caps themselves are a memory backstop only (200 000 each): overrunning one evicts the oldest entries down to the cap, still counted as data loss.
...
| | `orderbook_skew_gate_held` | Book-stream batches queued by the block-order merge, waiting for the other stream to deliver; zero in steady state |
